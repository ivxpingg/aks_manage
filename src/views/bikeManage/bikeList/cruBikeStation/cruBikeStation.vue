<template>
    <Modal class="cruBikeStation-container"
           v-model="modalValue"
           :title="title"
           :width="1000"
           @on-visible-change="onVisibleChange">
        <div class="modal-body">
            <div class="baidu-dom" v-if="type === 'add'" id="baidu_bikeStation_add"></div>
            <div class="baidu-dom" v-if="type === 'edit'"  id="baidu_bikeStation_edit"></div>
            <Form ref="form"
                  class="form"
                  :model="formData"
                  :rules="rules"
                  :label-width="100">
                <Row>
                    <FormItem label="停车场名称:" prop="bikeName">
                        <Input v-model.trim="formData.bikeName" placeholder="请输入名称" />
                    </FormItem>
                    ...
                    <FormItem label="总数量:" >
                        <Input  v-model="formData.totalNum" type="number" placeholder="总数量" />
                    </FormItem>
                </Row>
            </Form>
    ...
    export default {
        ...
        data () {
            return {
                ...
                rules: {
                    bikeName: [{ required: true, message: '停车场名称不能为空!', trigger: 'blur' }],
        ...
        watch: {
            bikeId(val) {
                if (val) {
                    this.formData.bikeId = val;
                    this.getData();
                }
                else {
                    this.formData.bikeId = '';
                }
            },
            modalValue: {
                immediate: true,
                handler (val) {
                    let that = this;
                    if (val && !this.map) {
                        let id = this.type === 'add' ? 'baidu_bikeStation_add' : 'baidu_bikeStation_edit';
